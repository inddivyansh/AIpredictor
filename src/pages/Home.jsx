...
import { Search, LogIn, UserPlus, PieChart, BarChart2, Cpu, Newspaper, Lightbulb, Home as HomeIcon } from "lucide-react";
...
const features = [
  {
    title: "Home",
    desc: "Dashboard overview and quick access.",
...
const Home = () => {
  ...
  const [expandedChartType, setExpandedChartType] = useState("line");
  ...
  return (
    <motion.div
      className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-950 p-0 flex flex-col"
      initial={{ opacity: 0, y: 15 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.3 }}
    >
      {/* Top Bar - flush with sidebar, no extra space */}
      <div className="w-full sticky top-0 z-30 bg-slate-800 border-b border-slate-700 flex items-center justify-between px-2 sm:px-6 py-3">
      ...
      <div className="w-full max-w-5xl mx-auto mt-6 px-2 sm:px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {features.map((f, i) => (
            <Link
              ...
              key={f.title}
              className={`
                group flex flex-col items-center justify-center rounded-2xl p-6 min-h-[140px] transition-all duration-200
                ${f.isHome
                  ? "border-2 border-blue-400 bg-transparent text-blue-400 font-bold shadow-none"
                  : "bg-gradient-to-br " + f.color + " text-white shadow-xl hover:scale-[1.03] hover:shadow-2xl"
                }
                ${location.pathname === f.to && !f.isHome ? "ring-2 ring-blue-400" : ""}
              `}
              style={f.isHome ? { background: "none", boxShadow: "none" } : {}}
            >
              <div className={`mb-2 text-3xl ${f.isHome ? "text-blue-400" : ""}`}>{f.icon}</div>
              <div className={`text-lg font-bold mb-1 ${f.isHome ? "text-blue-400" : "text-white"}`}>{f.title}</div>
              <div className="text-xs text-slate-200 text-center">{f.desc}</div>
            </Link>
          ))}
        </div>
      </div>

      {/* Market/Options Buttons */}
